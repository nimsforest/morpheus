	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				...
				if state == machine.ServerStateDeleting || state == machine.ServerStateStopped {
					return nil
				}
				return err
			}
...
// convertContainer converts a Docker container to a machine.Server
func convertContainer(container *dockerContainer, networkName string) *machine.Server {
...
// convertContainerState converts Docker container state to machine.ServerState
func convertContainerState(state dockerState) machine.ServerState {
	switch state.Status {
	case "created":
