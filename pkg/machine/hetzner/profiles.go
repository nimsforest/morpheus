...
func GetHetznerServerType(profile machine.MachineProfile) MachineTypeMapping {
	mappings := map[machine.MachineProfile]MachineTypeMapping{
		machine.ProfileSmall: {
			Primary: "cx22", // Preferred: shared CPU, 2 vCPU, 4GB RAM
			Fallbacks: []string{
				"cpx11", // Dedicated AMD - first fallback
				"cx32",  // Shared AMD with more resources - second fallback
			},
			Architecture: "x86",
		},
		machine.ProfileMedium: {
...
func (p *Provider) SelectBestServerType(ctx context.Context, profile machine.MachineProfile, preferredLocations []string) (string, []string, error) {
