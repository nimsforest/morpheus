// Package httputil provides HTTP client utilities with custom DNS resolution
// and TLS certificate handling for various environments including Termux/Android.
package httputil
...
// CreateHTTPClient creates an HTTP client with proper TLS configuration and
// DNS resolver fallback for various environments including Termux/Android.
...
func createCustomDialer() func(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	// This is needed because some environments (like Termux) have broken local DNS
